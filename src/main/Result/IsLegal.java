...
public class IsLegal {
    ...
    public static File isTable(String dbName, String tbName) {
        File file = new File("src/DatabaseSourse/" + dbName + "/" + tbName + "");
        if (!file.exists()) {
            System.out.println(tbName + "表不存在");
            return null;
        } else {
            file = new File("src/DatabaseSourse/" + dbName + "/" + tbName + "/" + tbName + "-config.xml");
            return file;
        }
    }
    ...
    public static boolean hasIndex(String dbName, String tbName) throws DocumentException {
        File file = new File("src/DatabaseSourse/" + dbName + "/" + tbName + "/" + tbName + "-config.xml");
        SAXReader saxReader = new SAXReader();
