...
public class InsertDataIntoTable {
    ...
    public static void insertIntoTable(String dbName, String tbname, List<String> tmp1, List<String> tmp2) throws DocumentException, IOException {
        ...
        if (insertable_element.selectNodes("insertable").size() > 0) {
            ...
            //创建写入对象，获取记录数量
            File write_file = new File("src/DatabaseSourse/" + dbName + "/" + tbname + "/" + write_file_name + ".xml");
...
//            }
            for (int i = 1; i < tmp1.size(); i++) {
                Element tmpElement = element.addElement(tmp1.get(i));
                tmpElement.setText(tmp2.get(i));
            }
            //若可插入文件记录数>=10,更新配置信息
            ...
            if (nodes.size() >= CreateTable.entry_num) {
                ...
                for (int i = 1; i < tmp1.size(); i++) {
                    Element tmpElement = newChild.addElement(tmp1.get(i));
                    tmpElement.setText(tmp2.get(i));
                }
                CreateTable.writeIO(new File("src/DatabaseSourse/" + dbName + "/" + tbname + "/" + newfile + ".xml"),newDocument);
            ...
            //否则在最后一条记录后插入
                ...
                for (int i = 1; i < tmp1.size(); i++) {
                    Element tmpElement = childElement.addElement(tmp1.get(i));
                    tmpElement.setText(tmp2.get(i));
                }
                CreateTable.writeIO(write_file,document);
