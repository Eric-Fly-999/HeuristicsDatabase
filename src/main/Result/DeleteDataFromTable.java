    ...
    public static void deleteFromTable(String dbName,String tbName,List<String> tmp) throws DocumentException, IOException {
        ...
        if(!IsLegal.isIndex(config_file,key)) {
            for (int j = Integer.parseInt(write_file_last_num); j >= 0; j--) {
                ...
                File file = new File("src/DatabaseSourse/" + dbName + "/" + tbName + "/" + tbName + last_num + ".xml");
        ...
        //主键查询删除
            ...
            File file=new File("src/DatabaseSourse/"+dbName+"/"+tbName+"/"+file_name+".xml");
...
//        System.out.println(key_value[0] + " " + key_value[1]);
        ...
        for (Node node : nodes) {
            ...
            List<Element> list = currentNode.elements();
            for (Iterator i = list.iterator(); i.hasNext(); ) {
                Element tmpnode = (Element) i.next();

                if(tmpnode.getName().equals(key_value[0]) && tmpnode.getText().equals(key_value[1])){
                    find = true;
                    break;
                }
            }
            if (find) {
                ...
                //更新配置文件
                if(isFull) {
                    File file1 = new File("src/DatabaseSourse/" + dbName + "/" + tbName + "/" + tbName + "-config.xml");
