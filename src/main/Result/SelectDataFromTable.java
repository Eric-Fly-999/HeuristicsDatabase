...
import main.DataStuctrue.BPlusTree;
import org.dom4j.*;
...
public class SelectDataFromTable {
    ...
    public static void selectFromTb(String dbName, String tbName) throws DocumentException {
        ...
        for (int j = Integer.parseInt(file_num); j >= 0; j--) {
            String num = "" + j;
            File file = new File("src/DatabaseSourse/" + dbName + "/" + tbName + "/" + tbName + num + ".xml");
            //解析xml
...
//            }
            for (Node node : nodes) {
                Element elementNode = (Element) node;
                List<Element> list = elementNode.elements();
                for (Iterator i = list.iterator(); i.hasNext(); ) {
                    Element tmpnode = (Element) i.next();
                    System.out.print(tmpnode.getName() + "=" + "\"" + tmpnode.getText() + "\"" + " ");
                }
                System.out.println();
            }
        }
    ...
    public static void selectAllFromTb(String dbName, String tbName, List<String> tmp1) throws DocumentException {
        ...
        for (int j = Integer.parseInt(file_num); j >= 0; j--) {
            ...
            for (Node node : nodes) {
                ...
                List<Element> list = node1.elements();
                for (Iterator i = list.iterator(); i.hasNext(); ) {
                    Element tmpnode = (Element) i.next();
                    if(tmpnode.getName().equals(condition[0]) && tmpnode.getText().equals(condition[1])){
                        condition_find = true;
                        find = true;
                        break;
                    }
            ...
            for (Node node : nodes) {
                ...
                for (Iterator i = list.iterator(); i.hasNext(); ) {
                    ...
                    for(int k = 0; k < tmp1.size(); k++){
                        if(tmpnode.getName().equals(tmp1.get(k))){
                            columns_find = true;
                            System.out.print(tmpnode.getName() + "=" + "\"" + tmpnode.getText() + "\"" + " ");
                            break;
                        }
...
//        }
    ...
    public static void selectWithIndex(String dbName, String tbName, List<String> tmp1, List<String> tmp2) throws DocumentException {
        ...
        File file = new File("src/DatabaseSourse/" + dbName + "/" + tbName + "/" + filename + ".xml");
        //解析XML
        SAXReader reader = new SAXReader();
        Document document = reader.read(file);
        Element rootElement = document.getRootElement();
