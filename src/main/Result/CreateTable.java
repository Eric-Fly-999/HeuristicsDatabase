...
public class CreateTable{
    static public int entry_num=500;
    ...
    public static void createTb(String dbName, String tbName, List<String> tmp) throws IOException {
        ...
        //创建一张表的文件夹，逻辑上表示一张表
        File tableFile=new File("src/DatabaseSourse/"+dbName+"/"+tbName+"");
        if(!tableFile.exists()){
            tableFile.mkdir();
        }
        else{
        ...
        //创建配置文件并设置根节点
        File table=new File("src/DatabaseSourse/"+dbName+"/"+tbName+"/"+tbName+"-config.xml");
        ...
        //创建表的物理层第一张子表
        File first_file=new File("src/DatabaseSourse/"+dbName+"/"+tbName+"/"+tbName+"0.xml");
        Document first_document=DocumentHelper.createDocument();
        Element first_rootElem = first_document.addElement(tbName+"s");
//        //根节点的属性值为列名称=数据类型
//        for (int i = 0; i < tmp.size(); i++) {
//            String[] list=tmp.get(i).split(" ");
//            first_rootElem.addAttribute(list[0],list[1]);
//        }
        //写入操作
